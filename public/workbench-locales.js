export const WORKBENCH_MESSAGES = Object.freeze({
  en: Object.freeze({
    ...
    "help.sync.1": "OpenGlance checks the remote when the repository opens and at the interval selected under Settings → General (10 minutes by default). A clean worktree fast-forwards automatically, and harmless local copies that exactly match files newly added remotely are adopted safely. Check now remains available. Entering Sync expands every folder leading to an unpublished file.",
  ...
  "zh-CN": Object.freeze({
    ...
    "help.sync.title": "同步",
    "help.sync.1": "OpenGlance 会在打开仓库时检查远端，之后按“设置 → 常规”中的频率继续检查（默认每隔 10 分钟）。工作区干净时会自动快进；如果本地仅提前存在与远端新增文件逐字节相同的副本，也会安全接管。同时保留“立即检查”，每次进入 Sync 都会自动展开所有包含未发布文件的目录链。",
