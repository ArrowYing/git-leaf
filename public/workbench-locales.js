export const WORKBENCH_MESSAGES = Object.freeze({
  en: Object.freeze({
    ...
    "repository.remove": "Remove from Git Leaf",
    "repository.removeCurrent": "Close and remove from Git Leaf",
    "toast.repositoryRemoved": "Removed {repository} from Git Leaf. Local files were not deleted.",
    ...
    "help.sync.1": "Git Leaf checks the remote when the repository opens and at the interval selected under Settings → General (10 minutes by default). A clean worktree fast-forwards automatically, and harmless local copies that exactly match files newly added remotely are adopted safely. Check now remains available. Entering Sync expands every folder leading to an unpublished file.",
    "help.sync.2": "Unpublished local edits pause automatic updating instead of being written through in the background. Merge remote changes is then an explicit action; a conflict-free protected merge advances the branch while keeping every local edit uncommitted. A safe automatic update affecting the focused Source or Live document stays pending until focus leaves the editor, so continued typing is never discarded. Sync and publish is the separate manual action that commits and pushes all repository changes.",
    ...
    "help.telemetry.1": "Official Git Leaf builds may send an anonymous installation identifier, app version, update status, daily active duration and foreground exposure, repository count, and aggregate core-feature counts to understand installation health and real usage.",
  ...
  "zh-CN": Object.freeze({
    ...
    "repository.remove": "从 Git Leaf 移除",
    "repository.removeCurrent": "关闭并从 Git Leaf 移除",
    "toast.repositoryRemoved": "已从 Git Leaf 移除 {repository}，本地文件未被删除。",
    ...
    "newFolder.inDirectory": "将在“{name}”中创建。Git Leaf 会加入 .gitkeep，让空文件夹也能进入 Git。",
    "newFolder.repoHome": "将在仓库首页创建。Git Leaf 会加入 .gitkeep，让空文件夹也能进入 Git。",
    ...
    "help.sync.title": "同步",
    "help.sync.1": "Git Leaf 会在打开仓库时检查远端，之后按“设置 → 常规”中的频率继续检查（默认每隔 10 分钟）。工作区干净时会自动快进；如果本地仅提前存在与远端新增文件逐字节相同的副本，也会安全接管。同时保留“立即检查”，每次进入 Sync 都会自动展开所有包含未发布文件的目录链。",
    "help.sync.2": "本地存在未发布编辑时，后台自动更新会暂停，不会越过这些编辑写入工作区。此时需要明确点击“合并远端修改”；受保护合并无冲突时会推进分支，同时让全部本地编辑保持未提交。可安全自动应用的更新若涉及当前聚焦的 Source 或 Live 文档，会等到焦点离开编辑区域再完成，因此继续输入不会丢失。“同步并发布”仍是另一个人工操作，才会提交并推送整个仓库的改动。",
    ...
    "help.telemetry.1": "Git Leaf 正式版会发送匿名安装实例、App 版本、更新状态、每日活跃时长与前台展示时长、仓库数量和核心功能次数，用于了解安装更新是否正常以及工具是否被实际使用。",
