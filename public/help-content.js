export const GIT_LEAF_HELP_SECTIONS = [
  {
    ...
    body: [
      ...
      "CSV、JSON、YAML、TXT、代码、配置和未知类型不会常驻内容目录。当前打开的文件、搜索结果，以及“仅本地改动”中的文件会按需临时显露。切换文档不会因为引用关系改变目录内容；全部仓库文件模式仍会显示完整仓库。",
  ...
  },
  {
    ...
    title: "分享文档",
    body: [
      "右上角“复制分享链接”只分享主工作区 main 中的 Markdown / MDX 文档，不会分享本机路径或 feature worktree。文档尚未发布时，确认“同步并复制”会完成提交、推送和远端复核，成功后直接把链接写入剪贴板。",
      ...
      "分享发布失败时，本地修改和已经创建的提交都会保留；检查网络、GitHub 登录或远端分支状态后可以原地重试，也可以把失败提示交给 AI Agent。分享链接不会授予 GitHub 权限；接收方本机尚未打开对应仓库时，Git Leaf 会要求选择本机目录并核对 GitHub origin。",
    ],
  },
  {
    id: "telemetry",
