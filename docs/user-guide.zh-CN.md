...
组件时会保持视觉呈现；通过工具栏的数据操作进入围栏正文，通过 `</>` 进入完整组件源码。Preview 不显示
MDX-lite 编辑工具栏。
...
日数据可切换日、周、月、季度，周数据只显示周。按钮只改变当前视图，不会改写报表或源数据；缺失源
数据周期和不完整展示周期会被明确提示。这些区间控件属于组件的呈现内容，不是 Live 编辑工具栏，也不会
选中组件。
