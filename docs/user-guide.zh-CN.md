...
拖动，进入相邻单元格后会按任意方向选中起点与终点围成的矩形区域。文字色板固定在表格上方，可为整个
选区设置或清除颜色，并可通过关闭按钮或 Esc 关闭。同一列纵向选中两个或更多单元格后，列顶部会显示
拖动把手；拖动它会移动整列，不要求先选满整列。这些操作只在 Live 中提供，Preview 仍然只读。写回
内容仍是原生管道表格和 Obsidian 可识别的内联颜色 `span`，不会变成 Git Leaf 私有的表格格式。
