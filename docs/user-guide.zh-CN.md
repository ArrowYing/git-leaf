...
OpenPeek 是供人使用的桌面 App，用来打开和维护团队与 AI Agent 共享的 Git 上下文仓库。它直接打开已经
...
1. 从[公开下载页](https://gitleaf.mangofuture.com/download?lang=zh-CN)安装 OpenPeek，或者使用
...
4. 通过目录树或搜索找到文档。OpenPeek 默认用 Preview 打开 Markdown 和 MDX。
5. 需要修改时再切换到 Live 或 Source。OpenPeek 会直接写入原文件并自动保存。

OpenPeek 没有单独的“保存”按钮。自动保存只代表修改已经写入本机；在发布以前，它仍然是尚未共享的本地
Git 改动。

## 认识 App 界面

![OpenPeek 展示仓库目录树、文档导航和易读的 Preview](assets/user-guide/browse-and-read.jpg)
...
下次启动时，OpenPeek 会恢复已经打开的仓库及其工作状态，包括 Tab、目录展开和阅读位置。
...
Markdown／MDX 文件名不含汉字时，OpenPeek 会在第一行保留真实文件名，并在 Frontmatter `title` 或
...
- **Favorites** 集中放置经常返回的文件夹和 Markdown／MDX 文档。可以从右键菜单收藏，也可以使用当前
  ...
  OpenPeek 会自动取消路径已经不存在的收藏；文件在外部改名后，可以在新位置重新收藏。
...
其他文件在被打开、发生改动或被搜索命中时出现。这个偏好只改变目录树显示，绝不会改变 OpenPeek 发现、
同步、提交或推送的范围。
...
OpenPeek 内打开；在 macOS 使用 Command-click，在 Windows 使用 Ctrl-click，可以把链接打开到另一个
Tab。
...
当前 Markdown／MDX 文件与最后一次提交的版本不同时，OpenPeek 会在 Preview、Source 和 Live 中显示
一条轻量的编辑轨迹：

- 当前新增或替换的正文与对应行号使用低干扰的暖色高亮；
- 每个受影响章节在文档导航中铺满整行底色，点击区域和跳转到原章节的行为保持不变；
- 首个可见导航标题以前的改动归入“文档顶部”，点击后返回文档开头；
- 打开其他文档再返回时，OpenPeek 会从仓库重新生成同样的提示，不需要维护另一份草稿标记。
...
尚未跟踪的新文档按空白版本比较。这个功能有意不做完整的 Git diff：它不显示 hunk，也不提供暂存、
丢弃或冲突解决。Sync 仍负责展示全部尚未发布的文件；文档内提示只帮助人快速定位并继续一次编辑。

## 选择合适的编辑方式

OpenPeek 始终编辑原来的 Markdown 或 MDX 文件。三种模式只是同一份源文件的不同界面：
...
可识别的受控内联样式，不会变成 OpenPeek 私有的表格格式。
...
OpenPeek 内只有 Markdown 和 MDX 可以编辑。其他仓库文件提供只读预览，或交给系统应用打开。普通只读
...
OpenPeek 不会用节点列表或自动生成的一跳小图替换它；需要参照作者原始布局时，可关闭“智能阅读”。
...
文本。AI Agent 能直接读取和修改源数据；OpenPeek 则把同一份源文件呈现为供人理解的视觉内容。
...
![OpenPeek 把 MDX 文档内的结构化数据呈现为图表](assets/user-guide/mdx-visuals.png)

这样可以避免截图或另一个仪表盘成为重要数据唯一的存在位置。Preview 呈现结果，Source 和 Live 仍然
编辑原文件。OpenPeek 只接受一组受控组件，不会运行任意 JSX、JavaScript、`import` 或文档脚本。
...
行号范围。上面的元数据只是示例，OpenPeek 实际复制的是当前工作目录的真实信息。最后一个来源说明之后，
...
OpenPeek 会在打开仓库时检查一次 Git 远端，之后按“设置 → 常规”中的频率继续检查。默认值为
10 分钟，可选 1、2、5、10、30、60、120 分钟：

- 本机工作目录干净、只是落后于远端时，OpenPeek 会安全地自动快进。
- 本地已有编辑时，OpenPeek 会自动应用能够无冲突合并的远端更新，全部本地修改仍保持未提交。
- 如果更新涉及当前聚焦的 Source 或 Live 文档，OpenPeek 会在后台完成准备，显示“远端更新待合并”，
  并在 OpenPeek 仍处于前台、焦点离开编辑区域后自动应用；切换到其他 App 会继续保持待合并。
...
阅读当前结果。OpenPeek 目前显示改动文件和当前文档内容，不提供完整的逐行 Diff 审核，也不会判断一处
改动来自哪个 Agent。

如果本地和远端历史已经分叉、出现冲突，或另一个 Git 操作正在进行，OpenPeek 会停止，不会改写历史，也
不会把未解决的合并留在真实工作目录。需要开发者级 Git 修复时，失败界面可以提供一段交给 AI Agent 的
提示词。

## 用 URL 打开本机对应文档

在线文档让协作显得简单，其中一个重要原因是：一个 URL 就能打开正确的文档。OpenPeek 保留这种点击即达
的便利，但文件与事实源仍然位于本地优先的 Git 仓库中。

Agent 把文档交给人检查时，完整流程是：

1. 仓库提示词要求 Agent 为需要检查的 Markdown／MDX 文件运行可信的链接生成器。
2. Agent 在最终回复中给出 HTTPS **Open in OpenPeek** 链接，而不是只给一个本机路径。
3. 浏览器打开 Mango Future 托管的 `/open` 中转页；第一次使用时，浏览器可能询问是否允许启动
   OpenPeek。
4. OpenPeek 根据 GitHub 仓库标识匹配本机 checkout，必要时请用户选择本机目录，然后打开指定文档。
...
本仓库提供了一份
[可以直接复制使用的独立生成器](../tools/generate-openpeek-open-link.mjs)，只依赖 Node.js 和 Git。
内容仓库可以把它放在同一路径，再在 `AGENTS.md` 等仓库提示文件中加入下面的通用写法：

```markdown
## OpenPeek 文档预览链接

最终回复需要让用户预览 Markdown 或 MDX 文件时，运行仓库自带的 OpenPeek 链接生成器：
...
`Open in OpenPeek: [<文档标题>](<返回的 HTTPS URL>)`

不要只返回本机绝对路径，也不要手工拼接 `/open`、`/share` 或 `openpeek://` URL。`/open` 只用于本机
定位和预览，不证明文件已经发布。需要把链接发送给其他人时，先把文档发布到 `main`，再使用 OpenPeek
的“复制分享链接”。
```
...
“复制分享链接”用于分享主工作目录 `main` 上的 Markdown／MDX 文档。文档尚未发布时，OpenPeek 会先
...
OpenPeek 可以保持多个仓库处于已打开状态。保存的仓库顺序会保持稳定，直到你主动调整；每个仓库分别恢复
自己的 Tab 和导航状态。
...
仓库行的操作菜单可以把仓库移出 OpenPeek。这个操作只会将它从已打开仓库列表中移除，不会删除本机目录、
Git 文件、分支或未发布修改；以后重新打开同一个仓库时，OpenPeek 会继续恢复已保存的工作区状态和收藏。
...
如果 worktree 当前没有分支，OpenPeek 会在第一次真正写入前创建保护分支，不会让修改停留在 Detached
HEAD。

## 打开其他仓库文件

OpenPeek 保持以文档为中心，但仍然让周边证据可以被查看：
...
同一页面还包含 OpenPeek 帮助、文件类型支持、环境与仓库状态，以及完整快捷键列表。在 macOS 使用
`Command+?`，在 Windows 使用 `Ctrl+?` 可以直接打开快捷键。

## 什么时候应该换用其他工具

当团队和 AI Agent 共享同一个 Git 仓库，而一部分对内容负责的人不希望采用开发者工作流时，OpenPeek
才是合适的人类界面。

详细 Diff、选择性暂存、创建分支、Rebase、解决冲突、重构代码和仓库管理，应使用 IDE 或 Git 客户端；
大范围修改和开发者级修复，应交给外部 AI Agent。OpenPeek 保持专注：让人能够阅读、检查、提供准确
上下文，并对同一批文件做范围明确的修正。

产品范围、下载方式和构建身份见 [OpenPeek README](../README.zh-CN.md)。
