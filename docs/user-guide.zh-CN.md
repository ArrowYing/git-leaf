...
在 Live 中按住鼠标拖过普通文字，会保留清晰的原生选区，并在选区上方打开紧凑的格式工具栏。粗体、
斜体、删除线、固定文字颜色、高亮色和清除格式都会作用于完整选区；macOS 可用 `Command+B`／`Command+I`，
Windows 可用 `Ctrl+B`／`Ctrl+I` 快速设置两种常用格式。写回内容仍是标准 Markdown，以及与表格相同的、
Obsidian 可识别的受控内联样式。选择其他 Live 对象会关闭文字工具栏，但不会改动已选文字。
