...
用“适应宽度”返回完整图形，用 `+`、`−` 缩放；放大后可拖动画布。`</>` 在图形和完全相同的 Mermaid
源码之间切换。这些控件只是临时阅读状态，不会改写文件。切换深色或浅色外观时，图形会按当前主题重新
渲染。语法无效或内容过大时，卡片会显示错误，同时保留 `</>` 供检查源码。

图是否易懂，首先由源码中的信息结构决定。方向单一、节点较少的过程使用自动布局的 `flowchart`；阶段较多
或包含反馈边的系统总览，使用 `block-beta` 明确控制少量、短标签的语义阶段，并用 `space` 留出真正的呼吸
空间；详细路径拆成较小的后续图。Git Leaf 会根据渲染比例调整画布高度，但不会静默改写作者表达的关系。
