...
文件名和标题使用相同的视觉分量，方便先看到其中任意一个。第二行默认开启；如需紧凑目录树，可在
“设置与帮助 → 文件与目录 → 仅显示文件名”中关闭。标题只用于展示；排序、链接、文件操作和 Git 始终
使用真实文件名与路径。
...
长期报表也可以把完整日数据放在文档旁边的 CSV、TSV 或 JSON 文件里，再用 `.dataset.json` 说明口径。
报表作者为 `Chart` 或 `DataTable` 指定日期区间后，读者可使用组件上方的按钮切换日、周、月、季度。
按钮只改变当前视图，不会改写报表或源数据；缺失日期和不完整周期会被明确提示。
