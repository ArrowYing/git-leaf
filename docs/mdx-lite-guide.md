---
title: OpenPeek MDX-lite renderer reference
domain: ai
type: guide
owner: maintainer
...
source: openpeek
canonical: true
ai_snippet: "[AI Reference] OpenPeek MDX-lite renderer | inline data and repository-local dataset views | DataTable Timeline Chart DecisionBox MetricGrid FlowDiagram"
---

# OpenPeek MDX-lite renderer reference

[Documentation index](README.md)

This is the authoritative renderer and editor reference for OpenPeek's MDX-lite capability. A repository
opened by OpenPeek can be any local Git repository; the implementation in OpenPeek defines the supported
...
execution. OpenPeek recognizes only the components documented here and renders their body data to static
HTML or SVG.
...
- Content repositories may maintain their own adoption rules and selected examples, but cannot override
  OpenPeek's implementation contract. The
...
`MetricGrid`, and `FlowDiagram`. In a `.md` document OpenPeek asks before renaming it to `.mdx`.
...
On error OpenPeek displays `MDX component failed to render: ...`; the source remains editable.
...
- Allowlisted component attributes may stay on one line or span multiple lines like HTML attributes.
  OpenPeek's insertion templates put one attribute on each line by default so longer components remain
...
120 plotted periods. If the configured or remembered interval is too dense, OpenPeek selects the finest
...
A weekly source period remains atomic when it is shown by natural month or quarter: OpenPeek assigns the
...
field has no rollup and multiple source rows enter a period, OpenPeek reports an error instead of
...
`DataTable`, `columns` controls order; without it, OpenPeek shows the time field plus fields that declare
...
Do not encode manual column widths. OpenPeek estimates them, keeps compact tables natural, scrolls wide
...
complete graph stays visible; OpenPeek does not synthesize isolated node lists or one-hop diagrams.
...
- Rely on OpenPeek layout instead of inventing width and positioning properties.
- Open the result in OpenPeek and inspect Preview or Live.
