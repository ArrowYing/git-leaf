...
文档导航也以 Tab 为边界：每个 Tab 有稳定身份、当前文档位置和独立的前进／后退栈。普通文件树或内部文档链接在当前
Tab 追加位置；Command／Ctrl 点击创建后台 Tab，Command／Ctrl 加 Shift 创建前台 Tab；切换 Tab 只激活目标 Tab，不写入
文档历史。顶部前进／后退与 `Command+[／]` 只移动活动 Tab 的历史位置，不能切换到其他 Tab、worktree 或仓库。浏览器 URL
只是当前 Tab 的可分享定位投影，不作为应用内文档历史；切换仓库只能通过明确的桌面入口完成。历史条目保存安全的仓库相对路径、
行定位 hash 和 Preview 滚动位置，并随 Tab 会话一并恢复；重命名会更新全部 Tab 的当前与历史路径。
