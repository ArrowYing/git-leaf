...
Git Leaf Windows 版目前通过未签名的自安装 ZIP 发布，不使用 Microsoft Store、MSI 或 MSIX，也不需要管理员权限。

由于可执行文件尚未进行 Authenticode 签名，Windows 可能显示未知发布者或 SmartScreen 警告。请只从 Git Leaf
...
- `git --version` 可以正常运行；
- 已经克隆到本机、准备由 Git Leaf 打开的 Git 仓库。

## 安装

1. 打开 `https://gitleaf.mangofuture.com/download?lang=zh-CN#windows`。
2. 下载 `GitLeaf-<version>-public-win32-x64.zip`。
3. 将 ZIP 的 SHA-256 与下载页公布的校验值进行比较。
4. 完全退出旧版 Git Leaf。
5. 完整解压 ZIP，不要直接从压缩包预览中运行。
6. 打开 `Git Leaf-win32-x64` 并运行 `Git Leaf.exe`。
7. Git Leaf 会把完整 App 复制到 `%LOCALAPPDATA%\GitLeaf\app`，创建开始菜单快捷方式，然后从固定位置重新启动。
8. 选择一个本机 Git 仓库。

Electron App 必须保留完整目录，不要只复制 `Git Leaf.exe`。
...
`Git Leaf Community`。它不会使用官方更新服务，默认关闭使用统计，也不应被描述成 Mango Future 官方发行版。
完整步骤见英文技术文档 [Build from source](build-from-source.md)。

## Deep Link

Git Leaf 从固定安装目录注册 `git-leaf://`：
...
1. 完全退出 Git Leaf。
2. 从开始菜单删除 Git Leaf 快捷方式。
3. 删除 `%LOCALAPPDATA%\GitLeaf\app`。
4. 只有在明确不再需要仓库列表、工作台会话和个人设置时，才删除 Git Leaf 的 Electron userData。
...
- App 从解压目录反复启动：确认 ZIP 已完整解压，并允许 App 写入 `%LOCALAPPDATA%\GitLeaf`。
- 更新提示重复出现：完全退出所有 Git Leaf 进程，再从开始菜单启动。
