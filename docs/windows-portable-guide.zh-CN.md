...
OpenPeek Windows 版目前通过未签名的自安装 ZIP 发布，不使用 Microsoft Store、MSI 或 MSIX，也不需要管理员权限。

由于可执行文件尚未进行 Authenticode 签名，Windows 可能显示未知发布者或 SmartScreen 警告。请只从 OpenPeek
...
- `git --version` 可以正常运行；
- 已经克隆到本机、准备由 OpenPeek 打开的 Git 仓库。

## 安装

1. 打开 `https://gitleaf.mangofuture.com/download?lang=zh-CN#windows`。
2. 下载 `OpenPeek-<version>-public-win32-x64.zip`。
3. 将 ZIP 的 SHA-256 与下载页公布的校验值进行比较。
4. 完全退出旧版 OpenPeek 或 Git Leaf。
5. 完整解压 ZIP，不要直接从压缩包预览中运行。
6. 打开 `OpenPeek-win32-x64` 并运行 `OpenPeek.exe`。
7. OpenPeek 会把完整 App 复制到 `%LOCALAPPDATA%\OpenPeek\app`，创建开始菜单快捷方式，然后从固定位置重新启动。
8. 选择一个本机 Git 仓库。

Electron App 必须保留完整目录，不要只复制 `OpenPeek.exe`。
...
Git Leaf 1.x 的固定安装升级到 OpenPeek 2.0 时，兼容可执行文件会接收一次旧更新器参数，把完整 App 迁移到
`%LOCALAPPDATA%\OpenPeek\app`。只有新可执行文件确认启动后，旧安装才会退出；如果旧更新器仍占用文件，
OpenPeek 会在下次启动时重试清理。
...
`OpenPeek Community`。它不会使用官方更新服务，默认关闭使用统计，也不应被描述成 Mango Future 官方发行版。
完整步骤见英文技术文档 [Build from source](build-from-source.md)。

## Deep Link

OpenPeek 从固定安装目录注册 `openpeek://`：
...
已有 `git-leaf://` 链接继续作为 1.x 兼容入口；OpenPeek 新生成的链接只使用 `openpeek://`。
...
1. 完全退出 OpenPeek。
2. 从开始菜单删除 OpenPeek 快捷方式。
3. 删除 `%LOCALAPPDATA%\OpenPeek\app`。
...
- App 从解压目录反复启动：确认 ZIP 已完整解压，并允许 App 写入 `%LOCALAPPDATA%\OpenPeek`。
- 更新提示重复出现：完全退出所有 OpenPeek 进程，再从开始菜单启动。
