...
OpenPeek is the people-facing desktop interface for a Git repository used as shared context by a team
...
1. Install OpenPeek from the [public download page](https://gitleaf.mangofuture.com/download), or use a
...
4. Use the directory tree or search to find a document. OpenPeek opens Markdown and MDX in Preview by
   default.
5. Switch to Live or Source only when you need to edit. OpenPeek writes those edits directly to the
...
![OpenPeek showing a repository tree, document outline, and readable Preview](assets/user-guide/browse-and-read.jpg)
...
OpenPeek restores open repositories and the working state of each repository on later launches,
...
For a Markdown or MDX filename written without Chinese characters, OpenPeek keeps the source filename
...
- **Favorites** collects folders and Markdown or MDX documents you return to frequently. Add an item
  ...
  `Ctrl+D` on Windows. After the complete repository tree refreshes successfully, OpenPeek automatically
...
matched by search. This preference changes only the tree display; it never changes what OpenPeek
...
but stays out of the outline. Internal document links stay inside OpenPeek; use Command-click on macOS
...
When the current Markdown or MDX file differs from its last committed version, OpenPeek presents a
...
OpenPeek always edits the original Markdown or MDX file. The three modes are different views of the
same source:
...
controlled Obsidian-compatible inline styles rather than an OpenPeek-only table format.
...
Only Markdown and MDX are editable inside OpenPeek. Other repository files are read-only previews or
...
horizontal source is presented vertically only after OpenPeek confirms that its node, relationship, and
cluster counts still match and no nodes overlap. An already vertical graph is never rotated horizontal
automatically. The complete vertical graph stays visible. OpenPeek does not replace it with a node list
...
ordinary CSV, TSV, JSON, or Markdown text. AI agents can read and update that source directly. OpenPeek
...
buttons change only the view and never rewrite the report or source data. OpenPeek calls out missing
...
![OpenPeek rendering a chart from structured data stored directly in an MDX document](assets/user-guide/mdx-visuals.png)

This avoids a screenshot or separate dashboard becoming the only place where important data exists.
Preview renders the result; Source and Live continue to edit the original file. OpenPeek accepts only a
...
````markdown
> 16 | OpenPeek opens a local Git repository and presents it as a readable workspace. The repository remains
> 17 | the shared source of truth. OpenPeek does not upload the content or create a separate hosted copy.
...
its own quoted lines and trailing source attribution. The values above are illustrative—OpenPeek copies
...
OpenPeek checks the configured Git remote when a repository opens and then on the interval selected
under **Settings → General**. The default is 10 minutes; the available intervals are 1, 2, 5,
10, 30, 60, and 120 minutes:

- If the local working directory is clean and only behind, OpenPeek safely fast-forwards it.
- If local edits also exist, OpenPeek automatically applies a conflict-free remote update while leaving
  every local edit uncommitted.
- If that update affects the focused Source or Live document, OpenPeek prepares it in the background,
...
other local changes. Open the affected documents to read the current result. OpenPeek currently shows
...
OpenPeek stops instead of rewriting history or leaving an unresolved merge in the real working
...
OpenPeek provides the same click-to-open convenience while the files and source of truth remain in a
...
2. The Agent returns an HTTPS **Open in OpenPeek** link instead of only a local path.
3. The browser opens Mango Future's `/open` handoff page and, on the first use, may ask permission to
   launch OpenPeek.
4. OpenPeek matches the GitHub repository identity to a local checkout, asks the user to choose one
...
[ready-to-copy standalone generator](../tools/generate-openpeek-open-link.mjs). It depends only on
...
```markdown
## OpenPeek document previews

When the final response should let the user preview a Markdown or MDX file, run the repository-owned
OpenPeek link generator:

node "$(git rev-parse --show-toplevel)/tools/generate-openpeek-open-link.mjs" \
  --repo-root "$(git rev-parse --show-toplevel)" \
  --file "<repository-relative.md-or-mdx>"

Use the returned HTTPS URL exactly in a Markdown link:
`Open in OpenPeek: [<document title>](<returned HTTPS URL>)`

Do not return only a local absolute path, and do not handcraft `/open`, `/share`, or `openpeek://`
URLs. `/open` is for local navigation and preview; it does not prove the file is published. For a link
sent to another person, first publish the document to `main`, then use OpenPeek's Copy share link.
```
...
working directory. If the document has unpublished changes, OpenPeek asks before **Sync and copy**
...
OpenPeek can keep several repositories open. Their saved order remains stable until you rearrange it, and
...
The row action can remove a repository from OpenPeek. This removes it only from the open repository
list: OpenPeek does not delete the local directory, Git files, branches, or unpublished changes. Opening
...
If a worktree has no branch, OpenPeek creates a protective local branch before the first real write.
It never leaves an edit on a detached Git commit.

## Open other repository files

OpenPeek keeps the document workflow focused while still making surrounding evidence available:

| Files | What OpenPeek does |
| --- | --- |
...
The same screen contains OpenPeek Help, supported file types, environment and repository status, and
the complete keyboard-shortcut list. Open shortcuts directly with `Command+?` on macOS or `Ctrl+?` on
Windows.

## When to use another tool

OpenPeek is the appropriate interface when a team and its agents share a Git repository, but some of
...
and developer-level repair. OpenPeek stays focused on helping people read, inspect, provide exact
context, and make bounded corrections to the same files.

For product scope, download options, and build identity, return to the [OpenPeek README](../README.md).
