...
# OpenPeek app usage analytics specification

## Authority

This is the sole contract for usage analytics in packaged OpenPeek desktop builds. It defines:
...
OpenPeek was named Git Leaf before version 2.0. The `git_leaf.*` event namespace is deliberately stable
across the rename because event names are persisted schema keys, not display copy. Clients must not emit
a parallel `openpeek.*` namespace or rewrite historical events; any future namespace migration requires
...
- Date-based cleanup implements retention. Policy changes update this specification and operations
  configuration first.
- Help states that OpenPeek may send anonymous installation, version, update, active-duration, and
...
3. updates receiver validation or distribution logging in `scripts/openpeek-update-server.py`;
...
| Receiver validation and download logging | `scripts/openpeek-update-server.py` |
