...
Git Leaf 保留文档表达的布局，不擅自猜测另一套拓扑。方向单一、节点较少的过程适合自动布局的
`flowchart`；阶段较多或包含反馈边的架构总览，适合使用 Mermaid `block-beta` 明确指定行列和 `space`
留白。阶段标签保持简短，详细路径拆成较小的后续图。适应画布和缩放只帮助检查，不能单独把一条过长的
拓扑变得易懂。
