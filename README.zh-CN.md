...
- **尚未发布的编辑始终容易找回。** Git Leaf 会在正文中高亮当前新增和替换的内容，在文档导航中为
  受影响章节铺满整行底色；还可通过默认关闭的开关查看带下划线的删除内容，以及“提交版本／当前文档”
  双行号。这是一条轻量的编辑轨迹，不是面向开发者的标准 Git diff。
...
2. **检查发生了什么变化。** Sync 显示尚未发布的本地文件和远端状态；打开相关文档后，正文直接高亮
   ...
   开关可进一步显示带下划线的删除内容和“提交版本／当前文档”双行号。
