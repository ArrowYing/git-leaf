...
- 侧边栏分为 All、Favorites、Sync 三个视图：All 是普通仓库目录树，Favorites 集中显示收藏的目录和 Markdown／MDX 文档，Sync 显示本地改动。
...
- 仓库级“同步”入口位于 Sync 视图，一次提交并推送当前仓库的全部本地改动，不要求选择文件或填写 Git 提交说明；分叉、冲突和进行中的 Git 操作会停止并交给 AI Agent 处理。
