const WINDOWS_INSTALL_PROGRESS_MESSAGES = Object.freeze({
  en: Object.freeze({
    ...
    "heading.update": "Updating OpenGlance{version}",
    "heading.start": "Starting OpenGlance{version}",
    "heading.install": "Preparing OpenGlance{version}",
    message: "Please wait. OpenGlance will finish automatically and reopen.",
    stage: "Starting…",
  }),
  "zh-CN": Object.freeze({
    "action.update": "更新",
    "action.start": "启动",
    "action.install": "准备",
    "heading.update": "正在更新 OpenGlance{version}",
    "heading.start": "正在启动 OpenGlance{version}",
    "heading.install": "正在准备 OpenGlance{version}",
    message: "请稍候，OpenGlance 会自动完成并重新打开。",
    stage: "正在开始…",
  }),
});
...
<head>
  ...
  <title>OpenGlance ${translate(`action.${actionKey}`)}</title>
  <style>
...
<body data-phase="starting">
  <main>
    <div class="brand"><div class="leaf">L</div><div class="name">OpenGlance</div></div>
  ...
  <script>
    window.updateInstallProgress = (state) => {
      ...
      document.getElementById("title").textContent = state.title || "OpenGlance";
