...
function renderLineSvg(model) {
  ...
  const { min, max, step } = valueRange(values, { allowNegative: values.some((value) => value < 0) });
...
function renderRightAxis(model, { min, max, step, y }) {
  return [
    ...
    ticks(min, max, step).map((tick) =>
      `<text class="mdx-chart-y-label mdx-chart-right-y-label" x="${model.width - model.pad.right + 8}" y="${(y(tick) + 4).toFixed(1)}" text-anchor="start" font-size="11">${formatAxisTick(tick, step)}</text>`
    ).join(""),
  ].join("");
}
...
function niceTickStep(roughStep) {
  ...
  const factor = [1, 2, 2.5, 5, 10].reduce((closest, candidate) => (
...
function ticks(min, max, step) {
  ...
  return Array.from({ length: count + 1 }, (_, index) => roundTick(min + step * index));
}
