...
test("desktop server starts the existing Git Leaf workbench on localhost", async () => {
  ...
  try {
    ...
    assert.match(html, /window\.GIT_LEAF_INITIAL_REPO = "git-leaf-desktop-/);
...
test("desktop server falls back when the preferred port is already serving another process", async () => {
  ...
  const desktopServer = await startDesktopGitLeafServer({ repoRoot, port: preferredPort });
