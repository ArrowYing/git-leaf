...
test("Windows update progress window defaults to English", () => {
  ...
  assert.match(html, /<title>OpenPeek Update<\/title>/);
  assert.match(html, /Updating OpenPeek 1\.4\.0/);
  ...
  assert.match(html, /Please wait\. OpenPeek will finish automatically and reopen/);
...
test("same-version redirect window accepts a Chinese locale alias", () => {
  ...
  assert.match(html, /正在启动 OpenPeek 1\.4\.0/);
  assert.match(html, /OpenPeek 会自动完成并重新打开/);
});
...
test("outdated package window starts in a visible handoff state", () => {
  ...
  assert.match(html, /Starting OpenPeek 1\.3\.0/);
...
test("Windows update progress window accepts the Chinese language option", () => {
  ...
  assert.match(html, /<title>OpenPeek 更新<\/title>/);
  assert.match(html, /正在更新 OpenPeek 1\.4\.0/);
  assert.match(html, /请稍候，OpenPeek 会自动完成并重新打开/);
