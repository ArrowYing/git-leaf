...
test("buildUpdateManifest describes macOS DMG and ZIP artifacts with absolute update URLs", () => {
  const manifest = buildUpdateManifest({
    ...
    artifacts: [
      { kind: "zip", fileName: "OpenPeek-0.1.2-darwin-arm64.zip", sha256: "abc", size: 123 },
      { kind: "dmg", fileName: "OpenPeek-0.1.2-darwin-arm64.dmg", sha256: "def", size: 456 },
    ],
  });
  ...
  assert.equal(
    ...
    "https://updates.mangofuture.com/git-leaf/stable/darwin-arm64/OpenPeek-0.1.2-darwin-arm64.zip",
  );
...
test("buildUpdateManifest can expose an ARM migration feed backed by universal artifacts", () => {
  const manifest = buildUpdateManifest({
    ...
    artifacts: [
      {
        kind: "zip",
        fileName: "OpenPeek-1.9.0-darwin-universal.zip",
        sha256: "abc",
        size: 123,
      },
    ],
  ...
  assert.equal(
    manifest.files.zip.url,
    "https://updates.mangofuture.com/git-leaf/stable/darwin-universal/OpenPeek-1.9.0-darwin-universal.zip",
  );
