...
test("buildGitSyncAgentPrompt localizes instructions without rewriting repository details", () => {
  ...
  assert.match(english, /^Please resolve this OpenPeek sync failure:/);
  ...
  assert.match(chinese, /^请处理 OpenPeek 同步失败：/);
...
test("syncSelectedFiles returns an all-file Agent prompt when rebase fails", async () => {
  ...
  assert.match(result.agentPrompt, /Preserve the OpenPeek user's changes/);
});
...
test("syncSelectedFiles publishes a branch without an upstream", async () => {
  const calls = [];
  const result = await syncSelectedFiles({
    repo: { ...REPO, branch: "openpeek/detached-1234567" },
    files: ["docs/changed.md"],
    gitRunner: async (_cwd, args) => {
      calls.push(args);
      const preflight = successfulPreflight(args);
  ...
  assert.deepEqual(calls.slice(-4), [[
    "push",
    "origin",
    `${TEST_HEAD}:refs/heads/openpeek/detached-1234567`,
  ], [
    "fetch",
    "origin",
    "openpeek/detached-1234567",
  ], [
    "merge-base",
    "--is-ancestor",
    TEST_HEAD,
    "refs/remotes/origin/openpeek/detached-1234567",
  ], [
    "branch",
    "--set-upstream-to=origin/openpeek/detached-1234567",
    "--",
    "openpeek/detached-1234567",
  ]]);
});
...
test("publishCurrentBranch accepts the language alias for localized failure copy", async () => {
  ...
  assert.equal(result.resultTitle, "同步遇到异常");
  assert.match(result.agentPrompt, /^请处理 OpenPeek 同步失败：/);
